...
const resourcesData = [
  {
    ...
    icon: Zap,
    items: [
      { title: "Panduan Metode STAR", desc: "Struktur jawaban situasi, task, aksi, hasil", badge: "Essential" },
      { title: "Teknik CAR Method", desc: "Context, Action, Result untuk jawaban ringkas", badge: "Popular" },
    ]
  },
  {
    ...
    icon: HelpCircle,
    items: [
      { title: "Behavioral Questions", desc: "50+ pertanyaan behavioral yang sering ditanya", badge: "50 Q&A" },
      { title: "Technical Questions", desc: "Pertanyaan teknikal untuk Software Engineer", badge: "40 Q&A" },
      { title: "Leadership Questions", desc: "Pertanyaan kepemimpinan untuk level Senior", badge: "25 Q&A" },
    ]
  },
  {
    ...
    icon: Star,
    items: [
      { title: "Mengatasi Gugup saat Interview", desc: "Teknik pernapasan dan mental preparation", badge: "Wellness" },
      { title: "Riset Perusahaan yang Efektif", desc: "Cara riset mendalam sebelum interview", badge: "Strategy" },
    ]
  }
];

export function Sidebar({ isOpen, onClose, onNewSession }: SidebarProps) {
  // State untuk section mana yang sedang terbuka
  const [openSection, setOpenSection] = useState<SidebarSection>("current");

  // State untuk Settings menggunakan LocalStorage
  ...
  // Simpan ke LocalStorage setiap kali appSettings berubah
  useEffect(() => {
  ...
  // Komponen Helper untuk Header Section
    ...
    return (
      ...
      >
        <div className="flex items-center gap-4">
          <div className={`p-2 rounded-xl transition-colors ${isActive ? 'bg-blue-600/20 text-blue-400' : 'bg-slate-800 text-slate-400 group-hover:text-blue-400'}`}>
            <Icon size={18} />
          </div>
  ...
  // Komponen Helper untuk Toggle Switch Custom
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="fixed left-0 top-0 h-full w-[320px] bg-[#0F141F] border-r border-slate-800/60 z-50 flex flex-col shadow-2xl overflow-hidden"
      >
        ...
        <div className="p-5 border-b border-slate-800/60 shrink-0">
          <div className="flex items-center justify-between mb-6">
            ...
            <button onClick={onClose} className="p-1.5 rounded-lg bg-slate-800 text-slate-400 hover:text-white lg:hidden">
              <X size={18} />
            </button>
          </div>
                ...
                <div className="px-5 pb-4">
                  <div className="bg-[#1A2130] rounded-2xl border border-slate-700/50 p-4 space-y-3">
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <div className="flex items-center gap-1.5 text-[#10B981]"><div className="w-2 h-2 rounded-full bg-[#10B981] animate-pulse" />Active</div>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                    ...
                    <div className="flex justify-between items-center text-sm">
                      ...
                      <span className="text-slate-200">Junior</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-400">Questions</span>
                      <span className="text-slate-200">1 / 10</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
                      <span className="text-slate-400">Duration</span>
                      <span className="text-slate-200 flex items-center gap-1"><Clock size={14} /> 05:24</span>
                    </div>
                    <div className="pt-2">
                      <div className="flex justify-between text-xs text-slate-500 mb-1.5"><span>Progress</span><span>10%</span></div>
                      <div className="h-1.5 w-full bg-slate-700 rounded-full overflow-hidden">
                        <div className="h-full bg-[#3B66F5] w-[10%] rounded-full" />
                      </div>
                    </div>
                    <div className="mt-3 p-2.5 rounded-xl bg-[#10B981]/10 border border-[#10B981]/20 flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-[#10B981]">Current Avg Score: Excellent</span>
                    </div>
                  </div>
                </div>
              </motion.div>
                  ...
                  {historyData.map((item) => (
                    <div key={item.id} className="bg-[#1A2130] rounded-2xl border border-slate-700/50 p-4 hover:border-slate-600 transition-colors cursor-pointer">
                      <div className="flex justify-between items-start mb-2">
                        <div>
                          <h4 className="text-slate-200 font-medium">{item.role}</h4>
                          <p className="text-slate-500 text-xs mt-0.5">{item.level} · {item.date}</p>
                        </div>
                ...
                <div className="px-5 pb-4">
                  <div className="bg-[#1A2130] rounded-2xl border border-slate-700/50 divide-y divide-slate-700/50">
                      ...
                      <div key={setting.id} className="flex items-center justify-between p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-slate-500 text-xs">{setting.sub}</p>
                          </div>
                        </div>
                        <ToggleSwitch 
                          checked={appSettings[setting.id as keyof typeof appSettings] as boolean} 
                          onChange={() => updateSetting(setting.id as keyof typeof appSettings, !appSettings[setting.id as keyof typeof appSettings])} 
                        />
                      </div>
                    ))}
                    ...
                    <div className="flex items-center justify-between p-4">
                      <div className="flex items-center gap-3">
                        <div className="p-2 rounded-lg bg-slate-800 text-slate-400"><Globe size={16} /></div>
                        <p className="text-slate-200 text-sm font-medium">Language</p>
                      </div>
                      <select value={appSettings.language} onChange={(e) => updateSetting('language', e.target.value)} className="bg-slate-800 text-slate-200 text-sm rounded-lg px-3 py-1.5 border border-slate-700 outline-none">
                    ...
                    <div className="flex items-center justify-between p-4">
                      <div className="flex items-center gap-3">
                        <div className="p-2 rounded-lg bg-slate-800 text-slate-400"><Zap size={16} /></div>
                        <p className="text-slate-200 text-sm font-medium">AI Model</p>
                      </div>
                      <select value={appSettings.aiModel} onChange={(e) => updateSetting('aiModel', e.target.value)} className="bg-slate-800 text-slate-200 text-sm rounded-lg px-3 py-1.5 border border-slate-700 outline-none">
                  ...
                  {resourcesData.map((group, idx) => (
                    <div key={idx}>
                      <div className="flex items-center gap-2 mb-3 px-1">
                        ...
                        <span className="text-slate-500 text-xs font-bold tracking-wider">{group.category}</span>
                      </div>
                      <div className="space-y-3">
                        {group.items.map((item, i) => (
                          <div key={i} className="bg-[#1A2130] rounded-2xl border border-slate-700/50 p-4 hover:border-slate-600 transition-colors cursor-pointer flex justify-between gap-4">
                            <div>
                              <h4 className="text-slate-200 font-medium text-sm mb-1">{item.title}</h4>
                              <p className="text-slate-500 text-xs leading-relaxed">{item.desc}</p>
                            </div>
                            <div className="flex flex-col items-end justify-between shrink-0">
                              <span className="bg-[#3B66F5]/20 text-[#3B66F5] text-[10px] font-bold px-2 py-1 rounded-md">{item.badge}</span>
